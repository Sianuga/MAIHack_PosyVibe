...
from eeg_system import start_eeg_system, send_start_command, send_stop_command, get_result_from_system, STANDARD_EEG_CONFIG

app = Flask(__name__)

# Start EEG system when Flask app starts
@app.before_first_request
def init_eeg():
    start_eeg_system(STANDARD_EEG_CONFIG)

@app.route('/eeg/start', methods=['POST'])
def start_eeg():
    send_start_command()
    return jsonify({"status": "started"})

@app.route('/eeg/stop', methods=['POST'])
def stop_eeg():
    send_stop_command()
...
# Initialize system
if start_eeg_system(STANDARD_EEG_CONFIG):
    print("EEG system ready!")
...
4. THREAD 3 INTEGRATION EXAMPLE:
---------------------------------
To add your emotion detection model as Thread 3:

1. Create your emotion model class:
   ```python
   class EmotionDetectionModel(threading.Thread):
       def __init__(self, input_queue, output_queue):
           super().__init__()
           self.input_queue = input_queue  # Gets filtered DataFrames
           self.output_queue = output_queue  # Sends predictions
           
       def run(self):
           while not self.stop_event.is_set():
               try:
                   processed_data = self.input_queue.get(timeout=1.0)
                   emotion, confidence = self.predict_emotion(processed_data.data_frame)
       ...
       def predict_emotion(self, df):
           ...
           # df contains 3 rows x 15 columns (time + 14 EEG channels)
           features = extract_features(df)
           prediction = your_model.predict(features)
           return prediction, confidence
   ```

2. Add to EEGSystemController.__init__():
   ```python
   self.emotion_model = EmotionDetectionModel(
       input_queue=self.filtered_eeg_queue,
       output_queue=self.emotion_results_queue
   )
   ```

3. Modify process_thread2_data() to use your model results
"""